...
下面 【一些 Hive SQL  的技巧】 以及 【一个Hive计算实例】

## 一些 Hive SQL  的技巧

关于Hive SQL ，一定需要有一个自己的自定义函数库，能有效的提高你的工作效率。我们封装了大量的UDF,UDAF,UDTF函数，通过这些函数，可以解决很多很复杂的问题。

我这里简单罗列几个：

1. hiveQL不能将子查询用于where条件里，所以要用到where条件in一个查询时，这是可以使用left semi join
，相当于in一个子查询
2. 
...
下面公式中，d 为文档，q 为query串，现在我们要形成如下格式的数据，用来做机器学习。
