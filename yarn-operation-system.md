...
2013年末以Docker为代表的容器技术，14年Google的容器管理平台Kubernetes 和 Contianer Engine 强势扩张，再到现在的Mesos 周边生态成熟，其提出的概念，DCOS(数据中心操作系统)开始炙手可热，多家公司在进行实践。尤其是用于偶颇具规模的互联网公司越来越多，各个组件比如异步通讯如消息队列Kafka，分布式协调组件Zookeeper等愈发成熟。已然形成了一套体系。其实你会发现，各公司的基础架构可能大同小异，用的框架项目等都差不多，所以现在以这些大家实践过的组件作为分布式式的系统实现，是比较现实也比较可取的。

比如，现在最火热的自动伸缩扩容，很多公司都在搞。在分布式操作系统这个体系里，其实异常简单。服务由Yarn开发的系统组件进行调度，这里，分布式系统里的应用模型本身就是容器，所以应该是非常易于调度的。容器被启动后注册到操作系统上标准API上(也就是Zookeeper)。交互层(nginx/haproxy)会自动被系统自动通知，将服务挂载上，完成透明的扩容操作。
